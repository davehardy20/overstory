 ...
 *
 * Implements IPlatformSpawner for Claude Code, leveraging the existing
 * tmux infrastructure from src/worktree/tmux.ts for robust session management.
 *
 * Features:
...
/**
 * Claude Code implementation of IPlatformSpawner.
 *
...
export class ClaudeSpawner implements IPlatformSpawner {
	...
	/**
	 * Spawn a new Claude Code agent process in a tmux session.
	 *
	 * Creates a detached tmux session running `claude` with:
	...
	async spawn(config: SpawnConfig): Promise<SpawnResult> {
		...
		const claudeArgs = this.buildClaudeArgs(config);
		const claudeCommand = `claude ${claudeArgs.join(" ")}`;

		const wrappedCommand =
			exports.length > 0 ? `${exports.join(" && ")} && ${claudeCommand}` : claudeCommand;
	...
	/**
	 * Build Claude CLI arguments based on spawn configuration.
	 */
	private buildClaudeArgs(config: SpawnConfig): string[] {
		const args: string[] = [];

		// Add --dangerously-skip-permissions for non-interactive automation
		// This is required for automated agent sessions
		args.push("--dangerously-skip-permissions");

		// Add model if specified in extra config
...
/**
 * Factory function to create a Claude spawner instance.
 *
 * @param projectName - Optional project name for session naming
 * @returns A new ClaudeSpawner instance
 */
export function createClaudeSpawner(projectName?: string): IPlatformSpawner {
	return new ClaudeSpawner(projectName);
}
