...
Project-agnostic swarm system for Claude Code and Opencode agent orchestration. Overstory turns a single AI agent session into a multi-agent team by spawning worker agents in git worktrees via tmux, coordinating them through a custom SQLite mail system, and merging their work back with tiered conflict resolution.

**Your AI agent session IS the orchestrator.** There is no separate daemon. CLAUDE.md/AGENTS.md + hooks + the `overstory` CLI provide everything. Supports both Claude Code and Opencode platforms.
...
When you open Claude Code or Opencode in a project with `.overstory/` initialized:
...
- **Layer 2 (Overlay):** Per-task context file (`CLAUDE.md` or `AGENTS.md`) generated by `overstory sling`, written to worktree, defining the WHAT (task ID, file scope, spec path, branch name)
...
```
Orchestrator (your AI agent session)
  --> Team Lead (AI agent in tmux, can spawn sub-workers)
        --> Specialist Workers (AI agent in tmux, leaf nodes)
```
...
Overstory supports multiple AI agent platforms. The platform is configured in `.overstory/config.yaml`.
...
platform:
  type: auto    # claude | opencode | auto
```

### Claude Code Setup

- **Context File:** `.claude/CLAUDE.md` (orchestrator) and per-worker worktrees.
- **Hooks Config:** `.claude/settings.local.json`
- **Hook Commands:** `overstory hooks install` (ensures hooks are in the right place for Claude)

### Opencode Setup

- **Context File:** `AGENTS.md` in the project root and per-worker worktrees.
- **Hooks Config:** `~/.config/opencode/hooks.json`
- **Hook Commands:** `overstory hooks install` (ensures hooks are in the right place for Opencode)
...
```
target-project/
  AGENTS.md                       # Opencode context file (if platform: opencode)
  .claude/CLAUDE.md               # Claude Code context file (if platform: claude)
  .overstory/
...
overstory coordinator <sub>            Persistent coordinator agent
  start                                  Start coordinator (spawns AI agent at root)
...
overstory hooks <sub>                  Manage orchestrator hooks
  install                                Install hooks to platform settings
...
overstory monitor <sub>                Manage Tier 2 monitor agent
  start                                  Start monitor (spawns AI agent at root)
