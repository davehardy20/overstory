...
Project-agnostic swarm system for Opencode agent orchestration. Overstory turns a single AI agent session into a multi-agent team by spawning worker agents in git worktrees via tmux, coordinating them through a custom SQLite mail system, and merging their work back with tiered conflict resolution.

**Your AI agent session IS the orchestrator.** There is no separate daemon. AGENTS.md + hooks + the `overstory` CLI provide everything.
...
- **Layer 2 (Overlay):** Per-task context file (`AGENTS.md`) generated by `overstory sling`, written to worktree, defining the WHAT (task ID, file scope, spec path, branch name)
...
platform:
  type: opencode    # Only opencode is supported
```
...
- **Hook Commands:** `overstory hooks install` (ensures hooks are in the right place)
...
overstory/                        # This repo (the overstory tool itself)
  src/
    ...
    metrics/
      ...
      transcript.ts               # Opencode transcript parser + cost estimation
  ...
  templates/
    AGENTS.md.tmpl                # Template for orchestrator AGENTS.md
...
```
target-project/
  AGENTS.md                       # Opencode context file
  .overstory/
