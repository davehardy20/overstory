...
1. **Receive the dispatch.** Your overlay (`.claude/AGENTS.md`) contains your task ID and spec path. The coordinator sends you a `dispatch` mail with task details.
...
- **On recovery**, reload context by:
  ...
  2. Reading your overlay: `.claude/AGENTS.md` (task ID, spec path, depth, parent)
...
Unlike the coordinator (which has no overlay), you receive your task-specific context via the overlay AGENTS.md at `.claude/AGENTS.md` in your worktree root. This file is generated by `overstory supervisor start` (or `overstory sling` with `--capability supervisor`) and provides:
